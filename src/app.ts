type Stage = "Graduated" | "Active Cohort";
...
function buildPeople(): Person[] {
  ...
  pushBatch(
    ...
    "Aug 11, 2023",
    "Graduated",
    "Graduation date recorded in the uploaded notebook."
  );
  ...
  pushBatch(
    ...
    "—",
    "Active Cohort",
    "Roster encoded; graduation column blank in the source page."
  );
  ...
  pushBatch(
    ...
    "—",
    "Active Cohort",
    "Roster encoded; completion and licensure updates may be added later."
  );
  ...
  pushBatch(
    ...
    "—",
    "Active Cohort",
    "Latest batch page encoded from the uploaded notebook."
  );
...
const heroMetricsEl = document.getElementById("heroMetrics") as HTMLDivElement;
const stageChart = document.getElementById("stageChart") as HTMLDivElement;
const encodedList = document.getElementById("encodedList") as HTMLDivElement;
const batchProgress = document.getElementById("batchProgress") as HTMLDivElement;
const batchCardGrid = document.getElementById("batchCardGrid") as HTMLDivElement;
const registryBody = document.getElementById("registryBody") as HTMLTableSectionElement;
const resultCount = document.getElementById("resultCount") as HTMLSpanElement;
const batchFilter = document.getElementById("batchFilter") as HTMLSelectElement;
...
function showView(targetView: string) {
      ...
      (batch) => `
        <article class="batch-card">
          ...
          <div class="batch-mini-grid">
            ...
            <div class="mini-tile"><strong>${batch.completed ?? "—"}</strong><span>Completed</span></div>
            <div class="mini-tile"><strong>${batch.graduated ?? "—"}</strong><span>Graduated</span></div>
...
function getFilteredPeople(): Person[] {
